...
- Include whole-project rebuild as a stronger destructive action with additional confirmation.
...
- [x] Compose actions are hidden when a container has no Compose metadata.
- [x] Service rebuild is available only when project/service metadata is validated.
- [x] Service restart is available only when project/service metadata is validated.
- [x] Whole-project rebuild has a stronger confirmation path than service actions.
- [x] Frontend cannot provide arbitrary compose commands or flags.
- [x] Contract docs, TS types, Rust serde structs, mocks, and tests are updated together.
